...
After OmniWM restarts, Hammerspoon waits up to 30 seconds for OmniWM IPC and
the restored Finder window. It also waits for an active Downloads shortcut to
finish. It then repairs one exact `~/Downloads` Finder window when its scratchpad
assignment is missing. The repair hides that window and restores the prior
focus. Hammerspoon changes nothing when the Finder state is ambiguous.
