...
Run the focused managed-hooks test, the relevant tmux state test, and `bin/provision`. Confirm in a live tmux pane that a resumed named Pi session changes the tab title to its restored session name.
