...
Wait up to 30 seconds for OmniWM IPC, restored-window discovery, and any active
Downloads shortcut after the Hammerspoon helper loads. Then run one bounded
recovery attempt. Continue only when scratchpad slot 1 is empty and exactly one
ordinary Finder window has the exact configured Downloads path as its title.
