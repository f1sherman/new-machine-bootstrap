...
A focused contract test will assert that the managed template contains `**/.pi/remote-pi/`, ignores runtime files at repository root and below nested working directories, and does not contain a blanket `.pi/` rule. After provisioning, `git check-ignore -v .pi/remote-pi/config.json` must resolve through the deployed global ignore file, and the primary checkout must remain clean.
