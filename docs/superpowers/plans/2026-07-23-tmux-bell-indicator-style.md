...
**Goal:** Keep inline PR-state dot colors as foreground colors when tmux highlights a window with a bell.

**Architecture:** Replace tmux's inherited reverse bell style with the same explicit cyan-background, black-foreground style used for current and activity-highlighted windows. Lock the behavior into the existing tmux configuration contract for both supported platforms.
