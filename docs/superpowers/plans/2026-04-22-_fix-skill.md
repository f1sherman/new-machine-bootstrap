...
Note: Partial fallback. Full `bin/provision` was blocked in this environment because Linux pre_tasks require passwordless sudo before the shared skill install tasks run. For Task 3, `/tmp/fix-skill-install-verify.yml` was used to execute the shared skill-copy tasks from `roles/common/tasks/main.yml` and verify exact installed-file matches; it did not verify the full provisioning entrypoint end-to-end.
...
```bash
cmp -s roles/common/files/config/skills/common/_fix/SKILL.md ~/.claude/skills/_fix/SKILL.md
cmp -s roles/common/files/config/skills/common/_fix/SKILL.md ~/.codex/skills/_fix/SKILL.md
...
Expected:
- both `cmp -s` commands exit `0` (installed files exactly match the managed source)
- all seven `rg` commands return matches from both installed files.
...
Note: Full `bin/provision` end-to-end was not verified in this environment (blocked by missing passwordless sudo). The shared skill-copy tasks and exact installed-file matches were verified via the Task 3 fallback.
