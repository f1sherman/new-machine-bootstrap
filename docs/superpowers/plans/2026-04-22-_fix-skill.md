...
Note: Full `bin/provision` was blocked in this environment because Linux pre_tasks require passwordless sudo before the shared skill install tasks run. For Task 3, `/tmp/fix-skill-install-verify.yml` was used to execute the same shared-skill copy tasks from `roles/common/tasks/main.yml` for verification.
