...
RENOVATE_CONFIG="$REPO_ROOT/renovate-global.json"
...
run_renovate_checks() {
  ...
  assert_contains "$RENOVATE_RUN_WORKFLOW" "configurationFile: renovate-global.json" "renovate workflow uses the non-reserved global config filename"
  assert_not_contains "$RENOVATE_RUN_WORKFLOW" "configurationFile: renovate.json" "renovate workflow no longer uses a repo-reserved config filename"
