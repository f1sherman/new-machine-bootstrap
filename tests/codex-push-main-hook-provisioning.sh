...
HOOK_TASK="Merge managed Codex push-to-main hook into ~/.codex/hooks.json"
...
hooks_script="$tmpdir/codex-push-hooks-task.sh"
...
assert_eq "$TASK_STATUS" "0" 'push-to-main hook task exits cleanly on first run'
assert_contains "$TASK_OUTPUT" 'changed' 'push-to-main hook task reports change on first run'
assert_eq "$(jq -r '.kept' "$hooks_file")" 'true' 'push-to-main hook task preserves top-level content'
assert_eq "$(jq -r '.hooks.PreToolUse | length' "$hooks_file")" '3' 'push-to-main hook task merges one managed entry'
assert_eq "$(jq -r '[.hooks.PreToolUse[] | select(.matcher == "Bash")] | length' "$hooks_file")" '2' 'push-to-main hook task preserves existing Bash matcher entries'
assert_eq "$(jq -r '[.hooks.PreToolUse[] | select(.matcher == "Bash" and any(.hooks[]?; .type == "command" and .command == "~/.local/bin/codex-block-git-push-main"))] | length' "$hooks_file")" '1' 'push-to-main hook task installs the managed command once'
assert_eq "$(jq -r '.hooks.Notification[0].hooks[0].command' "$hooks_file")" 'notify' 'push-to-main hook task preserves unrelated hook groups'
assert_mode_0600 "$hooks_file" 'push-to-main hook task writes 0600 on creation'
...
run_task_snippet "$HOOK_SNIPPET" "$hooks_script" env HOOKS_FILE="$hooks_file" bash
assert_eq "$TASK_STATUS" "0" 'push-to-main hook task exits cleanly on second run'
assert_contains "$TASK_OUTPUT" 'unchanged' 'push-to-main hook task reports unchanged on second run'
cmp -s "$hooks_snapshot" "$hooks_file" \
  && pass_case 'push-to-main hook task is idempotent on second run' \
  || fail_case 'push-to-main hook task is idempotent on second run' 'content changed on second run'
