...
pass, fail = assert_contains(COMMON_SKILL, "flat YAML-like entries", "skill documents the flat human output", pass, fail)
pass, fail = assert_contains(COMMON_SKILL, "`---` separators", "skill documents entry separators", pass, fail)
pass, fail = assert_contains(COMMON_SKILL, "output verbatim by default", "skill preserves helper output by default", pass, fail)
pass, fail = assert_contains(COMMON_SKILL, "Do not rewrite it into `Blocked` / `Active` sections", "skill forbids status-bucket rewrites", pass, fail)
