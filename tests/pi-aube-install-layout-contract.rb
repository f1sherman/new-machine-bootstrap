...
non_darwin_package = 'pi_package_root="$pi_root"'
...
[package_task, bin_task].each do |task|
  ...
  unless branch_index && darwin_index && else_index && package_index && manifest_index && name_index && version_index && identity_index && bin_index &&
    ...
    abort "managed Pi task must use explicit Darwin and validated direct non-Darwin package layouts"
  end
...
abort "package-link task must export the direct package root" unless package_task.include?('PI_PACKAGE_ROOT="$pi_package_root"') && package_task.include?('export PI_PACKAGE_ROOT')
...
abort "non-Darwin package root must resolve the direct mise package path" unless package_task.include?('Pathname.new(ENV.fetch("PI_PACKAGE_ROOT")).realpath')
...
abort "missing non-Darwin direct package resolver" unless non_darwin_resolver
...
Dir.mktmpdir("pi-aube-layout") do |dir|
  ...
  abort "non-Darwin resolver rejected the direct managed package: #{stderr}" unless status.success?
  expected = "#{pi_bin}\n#{package_root}\n"
  abort "non-Darwin resolver returned the wrong direct paths: #{stdout.inspect}" unless stdout == expected
