...
non_darwin_package = 'pi_package_root="$pi_root/lib/node_modules/@earendil-works/pi-coding-agent"'
...
[package_task, bin_task].each do |task|
  ...
  unless branch_index && darwin_index && else_index && package_index && manifest_index && name_index && version_index && identity_index && bin_index &&
    ...
    abort "managed Pi task must use explicit Darwin and validated non-Darwin global-prefix layouts"
  end
...
abort "package-link task must export the global-prefix package root" unless package_task.include?('PI_PACKAGE_ROOT="$pi_package_root"') && package_task.include?('export PI_PACKAGE_ROOT')
...
abort "non-Darwin package root must resolve the mise global-prefix package path" unless package_task.include?('Pathname.new(ENV.fetch("PI_PACKAGE_ROOT")).realpath')
...
abort "missing non-Darwin global-prefix package resolver" unless non_darwin_resolver
...
Dir.mktmpdir("pi-aube-layout") do |dir|
  version = "0.80.10"
  package_root = File.join(dir, "lib/node_modules/@earendil-works/pi-coding-agent")
  ...
  abort "non-Darwin resolver rejected the global-prefix managed package: #{stderr}" unless status.success?
  expected = "#{pi_bin}\n#{package_root}\n"
  abort "non-Darwin resolver returned the wrong global-prefix paths: #{stdout.inspect}" unless stdout == expected
