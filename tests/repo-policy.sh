...
CODEX_TRUST_HOOK="$REPO_ROOT/roles/common/files/bin/codex-trust-managed-hooks"
...
CLAUDE_PERMISSION_RULES="$REPO_ROOT/roles/common/vars/claude_permissions.yml"
CLAUDE_SECURITY_GUIDE="$REPO_ROOT/roles/common/vars/CLAUDE.md"
...
run_install_checks() {
  ...
  assert_contains "$COMMON_MAIN" "Set global aube version via mise (Linux)" "common Linux role activates aube globally"
  assert_contains "$COMMON_MAIN" "Set global aube version via mise (macOS)" "common macOS role activates aube globally"
  assert_contains "$COMMON_MAIN" "aube@{{ tool_versions.runtimes.aube }}" "common role installs the catalog-pinned aube version"
  assert_contains "$COMMON_MAIN" "use --global aube@{{ tool_versions.runtimes.aube }}" "common role enables bare aubx shims with the pinned aube version"
  assert_contains "$COMMON_MAIN" "register: common_aube_global_tools_node_path_linux" "common Linux aube path register is not reused by skipped macOS tasks"
  assert_contains "$COMMON_MAIN" "register: common_aube_global_tools_node_path_macos" "common macOS aube path register is not reused by skipped Linux tasks"
  assert_contains "$COMMON_MAIN" "common_aube_global_tools_node_path_linux.stdout" "common Linux aube installs use the Linux path register"
  assert_contains "$COMMON_MAIN" "common_aube_global_tools_node_path_macos.stdout" "common macOS aube installs use the macOS path register"
  assert_contains "$COMMON_MAIN" "AUBE_PARANOID=true aubx ccstatusline@" "common Claude statusline enables aube paranoid mode"
  assert_contains "$COMMON_MAIN" "minimumReleaseAgeExclude=@openai/codex" "common Codex latest installs exempt the reviewed package from release-age failures"
  assert_contains "$COMMON_MAIN" "minimumReleaseAgeExclude=@mariozechner/pi-coding-agent" "common pi latest installs exempt the reviewed package from release-age failures"
  assert_contains "$COMMON_MAIN" "until: common_codex_aube_install_result.rc == 0" "common Codex aube install retries advisory API failures"
  assert_contains "$COMMON_MAIN" "until: common_pi_aube_install_result.rc == 0" "common pi aube install retries advisory API failures"
  assert_contains "$COMMON_MAIN" "aubx ccstatusline@" "common Claude statusline runs through aubx"
  assert_not_contains "$COMMON_MAIN" "npx -y ccstatusline" "common Claude statusline does not run npx"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" "Install or update gsd-browser via mise aube" "mise node globals task installs gsd-browser"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" 'aube add -C "$aube_config_dir" -g --allow-low-downloads gsd-browser@latest' "mise node globals task uses aube for the reviewed gsd-browser package"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" 'AUBE_PARANOID=true' "mise node globals task enables aube paranoid mode"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" "minimumReleaseAgeExclude=gsd-browser" "mise node globals latest install exempts the reviewed package from release-age failures"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" "until: mise_node_global_tools_aube_install_result.rc == 0" "mise node globals task retries advisory API failures"
  assert_contains "$MISE_NODE_GLOBALS_TASKS" 'AUBE_GLOBAL_BIN_DIR="$node_bin"' "mise node globals task links global binaries into pinned Node bin"
  assert_not_contains "$MISE_NODE_GLOBALS_TASKS" 'npm" install -g' "mise node globals task does not run npm global installs"
  ...
  assert_contains "$MACOS_MAIN" 'ansible_facts["architecture"] in ["arm64", "aarch64"]' "macOS role keeps Intel Ruby source fallback"
  ...
  assert_contains "$MACOS_INSTALLS" "Remove llm default model pin" "macOS provisioning removes managed llm model pins"
  assert_not_contains "$MACOS_INSTALLS" "llm models default" "macOS provisioning does not pin the llm model"
  ...
  assert_contains "$SETUP" "sudo pmset -c sleep 0" "macOS setup disables AC sleep"
  assert_contains "$SETUP" "sudo pmset -a disablesleep 0" "macOS setup restores normal global sleep behavior"
  assert_not_contains "$SETUP" "disablesleep 1" "macOS setup does not disable sleep system-wide"
  assert_contains "$PLAYBOOK" "Verify AC sleep is disabled (macOS)" "playbook verifies AC sleep settings"
  assert_contains "$PLAYBOOK" "pmset -c sleep 0" "playbook AC sleep remediation includes sleep 0"
  assert_contains "$PLAYBOOK" "Verify system-wide sleep is enabled (macOS)" "playbook verifies normal global sleep behavior"
  assert_contains "$PLAYBOOK" "pmset -a disablesleep 0" "playbook global sleep remediation includes disablesleep 0"
  assert_not_contains "$PLAYBOOK" "disablesleep 1" "playbook does not require system-wide disablesleep"
  ...
  assert_contains "$COMMON_ZSH" 'AUBE_PARANOID=true aubx -p readability-cli readable "$1"' "zsh 2markdown uses paranoid aubx with readability-cli package and readable binary"
  assert_not_contains "$COMMON_ZSH" 'aubx readability-cli "$1"' "zsh 2markdown does not assume package name is the binary"
  assert_not_contains "$COMMON_ZSH" "npx readability-cli" "zsh 2markdown does not use npx"
  ...
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(AUBE_PARANOID=true aubx eslint *)" "Claude permissions allow eslint through paranoid aubx"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(AUBE_PARANOID=true aubx prettier *)" "Claude permissions allow prettier through paranoid aubx"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(AUBE_PARANOID=true aubx stylelint *)" "Claude permissions allow stylelint through paranoid aubx"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(npx eslint *)" "Claude permission cleanup removes legacy eslint npx allow"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(npx prettier *)" "Claude permission cleanup removes legacy prettier npx allow"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(npx stylelint *)" "Claude permission cleanup removes legacy stylelint npx allow"
  assert_yaml_query_empty "$CLAUDE_PERMISSION_RULES" '.claude_permissions.allow[]? | select(test("Bash\\(npx"))' "Claude permissions allow list does not allow npx"
  assert_contains "$CLAUDE_SECURITY_GUIDE" 'Bash(AUBE_PARANOID=true aubx eslint *)' "Claude security guide examples use paranoid aubx"
  assert_not_contains "$CLAUDE_SECURITY_GUIDE" 'Bash(npx' "Claude security guide examples avoid npx"
  ...
  assert_contains "$LINUX_TMUX_CONF" "set -s extended-keys off" "Linux tmux disables extended keys so local Codex panes keep root key bindings"
  assert_contains "$MACOS_TMUX_CONF" "set -s extended-keys off" "macOS tmux disables extended keys so local Codex panes keep root key bindings"
  assert_not_contains "$LINUX_TMUX_CONF" "set -g extended-keys on" "Linux tmux does not let local Codex panes request extended-key passthrough"
  assert_not_contains "$MACOS_TMUX_CONF" "set -g extended-keys on" "macOS tmux does not let local Codex panes request extended-key passthrough"
  assert_contains "$LINUX_TMUX_CONF" "is_vim_or_agent_key_passthrough" "Linux tmux pane navigation can pass through remote agent panes"
  assert_contains "$MACOS_TMUX_CONF" "is_vim_or_agent_key_passthrough" "macOS tmux pane navigation can pass through remote agent panes"
  assert_contains "$LINUX_TMUX_CONF" "bind-key -n 'C-h' if-shell -F \"\$is_vim_or_agent_key_passthrough\" 'send-keys C-h'  'select-pane -L'" "Linux tmux C-h forwards before local navigation"
  assert_contains "$MACOS_TMUX_CONF" "bind-key -n 'C-h' if-shell -F \"\$is_vim_or_agent_key_passthrough\" 'send-keys C-h'  'select-pane -L'" "macOS tmux C-h forwards before local navigation"
