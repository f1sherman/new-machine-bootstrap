...
COMMON_MISE_CONFIG="$REPO_ROOT/roles/common/templates/dotfiles/mise/config.toml"
...
run_install_checks() {
  ...
  assert_not_contains "$COMMON_MAIN" "Install or update pi-coding-agent via aube" "common role no longer manually installs pi via aube add"
  assert_yaml_matches "$CATALOG" '.tool_versions.runtimes.pi_coding_agent' '^[0-9]+\.[0-9]+\.[0-9]+$' "catalog pins pi-coding-agent"
