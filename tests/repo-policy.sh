...
run_install_checks() {
  ...
  assert_contains "$COMMON_ZSH" 'aubx -p readability-cli readable "$1"' "zsh 2markdown uses aubx with readability-cli package and readable binary"
  ...
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(aubx eslint *)" "Claude permissions allow eslint through aubx"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(aubx prettier *)" "Claude permissions allow prettier through aubx"
  assert_contains "$CLAUDE_PERMISSION_RULES" "Bash(aubx stylelint *)" "Claude permissions allow stylelint through aubx"
  ...
  assert_contains "$CLAUDE_SECURITY_GUIDE" 'Bash(aubx eslint *)' "Claude security guide examples use aubx"
