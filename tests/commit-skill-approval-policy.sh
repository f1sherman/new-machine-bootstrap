...
COMMIT_SH="$REPO_ROOT/roles/common/files/config/skills/common/p-commit/commit.sh"
...
assert_contains "$PERMISSIONS_YML" "- \"Bash(~/.claude/skills/p-commit/commit.sh *)\"" "Claude permissions still allow commit helper"
...
assert_contains "$BLOCK_COMMIT_HOOK" "Invoke the \`p-commit\` skill via the Skill tool instead" "git commit hook points at Claude's skill invocation"
assert_not_contains "$BLOCK_COMMIT_HOOK" "Use the /personal:commit skill instead" "git commit hook drops slash-syntax guidance"
assert_not_contains "$BLOCK_COMMIT_HOOK" "committing-changes" "git commit hook drops legacy skill name"
assert_not_contains "$BLOCK_COMMIT_HOOK" "personal:commit" "git commit hook drops legacy slash-syntax guidance"
