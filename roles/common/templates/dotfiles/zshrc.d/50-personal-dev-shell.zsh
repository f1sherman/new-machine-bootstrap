...
# `target` instead of `path`: zsh ties `path` array to `PATH` scalar, so
# `local path` blanks PATH inside the function and any subshells it spawns.
repo-start() {
  local target arg
  for arg in "$@"; do
    case "$arg" in
  ...
  target="$("$HOME/.local/bin/repo-start" "$@" --print-path)" || return $?
  cd "$target" || return $?
  printf '%s\n' "$target"
}
...
2markdown() {
  ...
  AUBE_PARANOID=true aubx -p readability-cli readable "$1" | pandoc --from html --to gfm --wrap=none --output page.md
}
...
EOF
  ...
  case "$action" in
    checkout)
      ...
      ;;
    cd)
      cd "$value" && worktree_sync_tmux_state
      ;;
    *)
