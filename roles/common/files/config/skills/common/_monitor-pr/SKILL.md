...
8. Inspect the returned JSON and handle it directly. For every `result_kind:"alert"` payload, inspect `action.next_action` and follow it before returning to passive monitoring.
   ...
   - `merged`: invoke `_clean-up` with the monitor's `REPO_DIR` and `HEAD_BRANCH`. If target branch, worktree, and remote cleanup succeeds, clear saved state with `bash ~/.local/share/skills/_pr-monitor/state.sh clear "$REPO_DIR"` and stop. Retained extra branches from the helper summary do not block monitor cleanup. If cleanup exits nonzero, keep monitor state and report the cleanup failure.
