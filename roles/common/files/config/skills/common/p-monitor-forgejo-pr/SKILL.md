---
name: p-monitor-forgejo-pr
description: Start a session-bound Forgejo PR monitor in one managed PTY session for up to 24 hours.
---
...
3. In that same managed PTY session, switch into `REPO_DIR` and run:
   ...
   bash ~/.local/share/skills/p-pr-monitor/run.sh \
     ...
     --state-cmd ~/.local/share/skills/p-pr-forgejo/state.sh \
