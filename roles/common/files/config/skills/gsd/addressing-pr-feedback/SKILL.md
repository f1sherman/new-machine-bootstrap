...
- **`~/.gsd/agent/skills/addressing-pr-feedback/fetch-pr-comments.sh`** — Fetches all comments (inline review + top-level issue) with file/line metadata. Takes a PR number as its only argument.
- **`~/.gsd/agent/skills/addressing-pr-feedback/post-comment.sh`** — Posts inline replies to review comments or top-level comments on the PR. Supports `--quote` for attributing top-level replies.
...
```bash
~/.gsd/agent/skills/addressing-pr-feedback/fetch-pr-comments.sh PR_NUMBER
```
...
```bash
~/.gsd/agent/skills/addressing-pr-feedback/post-comment.sh \
  --pr PR_NUMBER \
...
2. Commit using the commit skill's helper script:
   ```bash
   ~/.gsd/agent/skills/committing-changes/commit.sh -m "Address review feedback: brief description" file1 file2 ...
   ```
