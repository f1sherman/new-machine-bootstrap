...
function resolvedCandidate(candidate, fallbackCwd, followFinalSymlink = true, resolving = new Set()) {
  ...
  if (finalEntry?.isSymbolicLink()) {
    ...
    if (resolving.has(absolute)) return null;
    resolving.add(absolute);
    const target = fs.readlinkSync(absolute);
    return resolvedCandidate(path.resolve(path.dirname(absolute), target), fallbackCwd, true, resolving);
  }
...
async function firstProtectedRoot(pi, candidates, cwd, options = {}) {
  for (const candidate of candidates) {
    ...
    if (await isAgentStatePath(pi, resolved, cwd)) continue;
