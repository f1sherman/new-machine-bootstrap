...
const CODEX_MANAGED_CHILD_MODEL = "openai-codex/gpt-6-luna";
const OPENAI_MANAGED_CHILD_MODEL = "openai/gpt-6-luna";
...
const SESSION_GOAL_INSUFFICIENT_CONTEXT = "NEEDS_CONTEXT";
const SESSION_GOAL_CHILD_SYSTEM_PROMPT = `Return one concise session name in this form: shortest recognizable subject + broad lasting outcome. Front-load the distinctive system or capability terms the user would use to find the session. Compress or omit redundant subject qualifiers so the subject and outcome fit within the first 31 characters when practical. The name must still describe why the session exists if the current symptom or task disappears. Treat a subordinate while clause as current detail unless it changes the broad objective. Do not name a symptom, implementation detail, phase, workflow, or next action. A ticket, repository, branch, PR number, or other identifier can appear only as secondary context after the name is recognizable without it. If the prompt supplies an opaque identifier without enough subject or description to create an independently recognizable name, output exactly ${SESSION_GOAL_INSUFFICIENT_CONTEXT}. Prefer at most 40 characters and never exceed 80. Examples: make the Pi OpenAI compaction extension reliable while fixing overlay replay becomes Pi compaction reliability; improve Safari URL routing while ignoring companion panels becomes Safari URL routing reliability; make repository cleanup safe while repairing merge proof becomes Repository cleanup safety; improve terminal workspace restoration while debugging stale manifests becomes Workspace restore reliability. Output only one unquoted line with no prefix or explanation.`;
...
export default function managedHooks(pi) {
  ...
  pi.registerTool({
    ...
    description: "Set the durable broad name and automatic identity for the current Pi session. Call once after you understand each new top-level objective, even if the session has a provisional automatic name. If the request cites an accessible issue, ticket, PR, branch, commit, or other opaque reference, inspect its content before calling this tool. Name the referenced work's recognizable subject and broad outcome so the name is useful without recalling the identifier. Use the form: shortest recognizable subject + broad lasting outcome. Front-load the distinctive system or capability terms the user would use to find the session. Compress redundant qualifiers so the subject and outcome fit within the first 31 characters when practical. The name must still describe why the session exists if the current symptom or task disappears. Do not name a symptom, implementation detail, phase, workflow, or next action. An identifier can appear only as optional secondary context after the name is recognizable without it. Keep the name for all work that contributes to the same objective, including scope refinement, implementation, debugging, testing, deployment, review, PR work, and discovered issues. A direct user rename is authoritative for the current objective; do not overwrite it during related work. Call again only when the user starts a clearly unrelated top-level objective or directly requests another name. Example: make the Pi OpenAI compaction extension reliable while fixing overlay replay becomes Pi compaction reliability. Prefer at most 40 characters (maximum 80).",
    parameters: {
      type: "object",
      properties: {
        name: { type: "string", description: "Descriptive subject from the request or inspected reference first, then broad outcome; recognizable without an identifier; include both in the first 31 characters when practical; prefer at most 40 characters (maximum 80)" },
      },
      required: ["name"],
