...
5. Extract `REPO_DIR="$(echo "$CONTEXT_JSON" | jq -r '.repo_dir')"` and `BRANCH="$(echo "$CONTEXT_JSON" | jq -r '.branch')"` and `BASE="$(echo "$CONTEXT_JSON" | jq -r '.base')"` and `BASE_REF="$(echo "$CONTEXT_JSON" | jq -r '.base_ref')"` and `PLATFORM="$(echo "$CONTEXT_JSON" | jq -r '.platform')"`.
6. Reject the request if `BRANCH` already matches `BASE`.
7. Invoke `_review` with the resolved `REPO_DIR` and `BASE_REF`, and let it classify and handle findings. `BASE_REF` is only for review; keep `BASE` for PR creation.
...
- The first review pass must use the resolved `REPO_DIR` and `BASE_REF` so review, proof, and body context all compare against the same commit range even when a stale local base branch exists.
