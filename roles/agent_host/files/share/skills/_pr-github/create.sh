...
write_pr_cache() {
  ...
  bash "$cache_helper" write \
    ...
    --state open \
    --head-sha "$sha" \
    --source _pr-github >/dev/null 2>&1 || true
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --head) head="$2"; shift 2 ;;
    -h|--help)
      echo "Usage: _pr-github --title \"PR title\" --body \"Description\" [--base branch] [--head branch]"
...
write_pr_cache "$created_number" "$created_url"
