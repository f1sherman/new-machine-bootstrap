...
module CodingAgentSyncer
  ...
  class << self
    ...
    def sync_to_codespace(source_dir, codespace_name, remote_path)
      ...
      # Use tar over ssh for reliable transfer with --skip-old-files for append-only behavior
      ...
      unless status.success?
        ...
        raise SyncError, "Tar transfer failed with exit code #{status.exitstatus}"
      end
